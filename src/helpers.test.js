...
describe("gets a random song selection", () => {
  let songInfo = helpers.getRandomSong();
  let possibleTitles = ["bigPoppa", "putItOn", "californiaLove", "msJackson"];
  let possiblePaths = [
...
describe("gets the path of a platform image depending on a platform's length", () => {
  let validKeys = ["plank1", "plank2", "plank3"];
  let longPlatformLength = 201;
  let shortPlatformLength = 199;
  let randomPlatformLength = Phaser.Math.Between(100, 400);
